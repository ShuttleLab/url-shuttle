...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (locale === "zh") {
    const t = await getTranslations({ locale, namespace: "toolPages.queryStringEditor" });
    return {
      title: t("title"),
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="text-sm text-muted-foreground text-center">
          <Link href="/tools/query-string-editor/" className="underline hover:text-foreground">
            查看完整英文指南 →
          </Link>
        </p>
      </div>
